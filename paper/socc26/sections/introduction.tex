...
flexible coded learning.  Its coded arm assigns a target-specific importance
score to encoded rows, aggregates scores over row pairs, maps decode-critical
pairs to workers predicted to finish early, and runs the same row-span test as
the decoder to predict the first-decodable prefix.  A fixed online guard enables
the adaptive arm only when worker-speed mismatch, predicted first-decode gain,
prefix growth, and rows-after-decode counters pass; otherwise \textsc{G-port}
falls back to static sparse-flexible placement.  The design keeps the
