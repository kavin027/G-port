...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run a multi-seed two-node TCP validation sweep through the SSH "
            "tunneled remote-worker runtime, then analyze the outputs."
        )
    )
    ...
    parser.add_argument("--output-root", type=Path, default=Path("tunneled_remote_sweep"))
    parser.add_argument("--diagnostics-out", type=Path, default=Path("tunneled_remote_diagnostics"))
    ...
    parser.add_argument("--local-base-port", type=int, default=27300)
    ...
    parser.add_argument("--remote-out-prefix", default="tunneled_remote_seed")
    parser.add_argument("--remote-base-port", type=int, default=28400)
    parser.add_argument("--port-stride", type=int, default=100)
    return parser.parse_args()


def _append_flag(cmd: list[str], name: str, value: object) -> None:
    cmd.extend([name, str(value)])


def build_run_command(args: argparse.Namespace, seed: int, index: int, out_dir: Path) -> list[str]:
    local_base_port = args.local_base_port + index * args.port_stride
    remote_base_port = args.remote_base_port + index * args.port_stride
    cmd = [sys.executable, "run_tunneled_remote_network_experiment.py"]
    for name, value in [
        ...
        ("--local-base-port", local_base_port),
...
def build_analyze_command(args: argparse.Namespace, out_dirs: list[Path]) -> list[str]:
    return [
        sys.executable,
...
def main() -> None:
    ...
    for index, seed in enumerate(args.seeds):
        ...
        print(
            "\n== Running seed "
            f"{seed} with local_base_port={args.local_base_port + index * args.port_stride} "
            f"remote_base_port={args.remote_base_port + index * args.port_stride} =="
        )
        print(" ".join(str(part) for part in cmd))
        subprocess.run(cmd, check=True, env=env)

    if args.analyze:
        cmd = build_analyze_command(args, out_dirs)
        print("\n== Analyzing tunneled remote sweep ==")
        print(" ".join(str(part) for part in cmd))
        subprocess.run(cmd, check=True, env=env)
